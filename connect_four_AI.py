...
pygame.init()
pygame.display.set_caption('Connect4 | code-monk08')
...
def draw_board(board):
    for c in range(cols):
        for r in range(rows):
            ...
            pygame.gfxdraw.aacircle(screen, int(
                c*squaresize + squaresize/2), int((r+1) * squaresize + squaresize/2), radius, black)
    ...
    for c in range(cols):
        for r in range(rows):
            if board[r][c] == 1:
                redcoin(int(c*squaresize) + 5, height -
                        int(r * squaresize + squaresize - 5))
                pygame.mixer.music.load('disc_drop1.wav')
                pygame.mixer.music.play(0)
            elif board[r][c] == 2:
                yellowcoin(int(c*squaresize)+5, height -
                           int(r * squaresize + squaresize - 5))
...
def undo_move(board, row, col):
    ...
    blackcoin(int(col*squaresize)+5, height -
              int(row * squaresize + squaresize - 5))
    print_board(board)
...
while not game_over:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            posx = event.pos[0]
            if turn == 0:
                redcoin(posx - (squaresize/2),
                        int(squaresize) - squaresize + 5)
            else:
                yellowcoin(posx - (squaresize/2),
                           int(squaresize) - squaresize + 5)
        pygame.display.update()
